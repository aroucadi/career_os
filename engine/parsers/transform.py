...
def convert_executive_signals_to_text(signals: dict) -> str:
    """Format extracted executive signals for the LLM prompt."""
    if not signals:
        return ""

    text = "\n\n=== EXECUTIVE & LEADERSHIP SIGNALS ===\n"

    if signals.get("pnl_and_budget_scale"):
        text += f"💰 P&L / Budget Signals: {', '.join(signals['pnl_and_budget_scale'])}\n"

    if signals.get("org_scale_and_headcount"):
        text += f"🏛️ Organizational / Headcount Scale: {', '.join(signals['org_scale_and_headcount'])}\n"

    if signals.get("governance_and_compliance"):
        text += f"⚖️ Governance & Risk Frameworks: {', '.join(signals['governance_and_compliance'])}\n"

    if signals.get("c_level_and_board_exposure"):
        text += f"🤝 C-Suite & Board Stakeholder Signals: {', '.join(signals['c_level_and_board_exposure'])}\n"

    if signals.get("custom_apps_and_innovation"):
        text += f"🚀 Product Innovation & Internal Tooling: {', '.join(signals['custom_apps_and_innovation'])}\n"
