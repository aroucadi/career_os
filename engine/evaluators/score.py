...
from transform import (
    ...
    convert_executive_signals_to_text,
)
from executive_enricher import extract_executive_signals
...
def _evaluate_resume(
    ...
    # Inject Raw Job Description if provided
    if jd_path and os.path.exists(jd_path):
        jd_content = Path(jd_path).read_text(encoding="utf-8")
        resume_text = (
            f"=== TARGET JOB DESCRIPTION FOR MANDATE COMPATIBILITY AUDIT ===\n"
            f"{jd_content}\n"
            f"===============================================================\n\n"
            + resume_text
        )

    # Extract & append Executive/Leadership Signals
    executive_signals = extract_executive_signals(resume_data)
    executive_text = convert_executive_signals_to_text(executive_signals)
    if executive_text:
        resume_text += executive_text


    # Add GitHub data if available and required by role
    if github_data and role.requires_github:
...
def main(pdf_path, role: Role, jd_path: str = None):
    ...
    if not github_cache_loaded and role.requires_github:
    ...
    score = _evaluate_resume(resume_data, role, evaluation_model, github_data=github_data, jd_path=jd_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--jd",
        metavar="JD_PATH",
        help="Optional path to a raw Job Description text/markdown file for context-aware scoring.",
    )
    parser.add_argument(
        "--init-role",
        metavar="NAME",
    ...
    main(args.pdf_path, role, jd_path=args.jd)
